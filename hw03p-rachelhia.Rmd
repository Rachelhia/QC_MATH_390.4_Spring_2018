...
str(diamonds)
```
The diamonds dataset contains the prices and other attributes of almost 54,000 diamonds. The attributes include carat, cut, color and clarity. 

What is $n$, $p$, what do the features mean, what is the most likely response metric and why?
n is 53940 and p is 10
Response metric would be price because we generally gather attributes about a sales item to predict price. 
...
graph1 = ggplot(diamonds, aes(x = z, y = price)) +
  ggtitle("Price by Depth, Cut & Color", subtitle = "from the diamonds dataset") +
  ylab("Price (in $'s)") +
  xlab("depth") +
   xlim(0, 10) +
...
We used a categorical predictor instead of a continuous one
...
summary(mod3)$sigma
```
Using the empirical rule, 95% of the predictions will be within a $2,260 range of the true value. 

Interpret all entries in the vector $b$.
The intercept is color D, cut Fair, clarity I1. Each slope for each variable is the increase or decrease in price from that variable. For example, an increase in carat would be $2,184 plus the intercept $11,256.
**TO-DO
...
Not particularly, there is a large enough sample size that this model could probably fit any diamond we'd want to predict the price of. There is a large n and we have a more degrees of freedom than we used.
...
it could be better as a polynomial probably but not terrible as linear.
...
The intercept is color D, cut Fair, clarity I1. Each slope for each variable is the increase or decrease in price from that variable. carat^2 is negative because when we extrapolate a polynomial it will eventually go down. This is the main difference between mod4 and mod3. 
**TO-DO

Is this an improvement over the model in #4? Yes/no and why.
Yes because the rmse is lower and r-squared is higher
**TO-DO
...
expect_equal(as.vector(t(yhat) %*% e),0, tol = 1e1) #(b)
...
an error within the range of $1300 is pretty significant so we probably dont want this model.
**TO-DO
...
Yes, g get closer because we have more degrees of freedom
**TO-DO
...
We don't need anymore because we would be overfitting even more. 
...
We could add the place that the diamond is being sold. A tiffany diamond is probably going to be more expensive than a costco diamond, even if it's the same exact diamond. 
